...
export const getTimeLogsByRepId = asyncHandler(async (req, res) => {
...
export const getTimeLogsSummary = asyncHandler(async (req, res) => {
  const { startDate, endDate } = req.query;
  const query: any = {};
  ...
  timeLogs.forEach((log: any) => {
    ...
    if (!summaryMap.has(repId)) {
      summaryMap.set(repId, {
        ...
        payType: log.rep.payType || "hourly",
        hourlyRate: log.rep.hourlyRate || null,
        semiMonthlyAmount: log.rep.semiMonthlyAmount || null,
        totalHours: 0,
        totalMinutes: 0,
        daysWorked: 0,
        logs: [],
  ...
  const summary = Array.from(summaryMap.values()).map((rep) => {
    ...
    return {
      ...
      semiMonthlyAmount: rep.semiMonthlyAmount,
      totalHours,
...
export const getTimeLogsSummaryByRepId = asyncHandler(async (req, res) => {
  const { startDate, endDate } = req.query;
  const query: any = { rep: req.params.repId };
